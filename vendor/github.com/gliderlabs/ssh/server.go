...
func (srv *Server) ensureHostSigner() error {
	...
	if len(srv.HostSigners) == 0 {
		fmt.Println("SSH: generateSigner")
		signer, err := generateSigner()
		if err != nil {
			return err
		}
		fmt.Println("SSH: generateSigner done")
...
func (srv *Server) Serve(l net.Listener) error {
	srv.ensureHandlers()
	fmt.Println("SSH: handlers are set")
	defer l.Close()
	if err := srv.ensureHostSigner(); err != nil {
		return err
	}
	fmt.Println("SSH: host signer is set")
	if srv.Handler == nil {
	...
	fmt.Println("SSH: track listener")
	...
	for {
		...
		fmt.Println("SSH: handle conn")
		go srv.HandleConn(conn)
		fmt.Println("SSH: conn handled")
	}
}
...
func (srv *Server) HandleConn(newConn net.Conn) {
	...
	fmt.Println("SSH: new server conn")
	if err != nil {
...
func (srv *Server) ListenAndServe() error {
	addr := srv.Addr
	fmt.Printf("SSH: starting server %s\n", addr)
	if addr == "" {
		addr = ":22"
	}
	ln, err := net.Listen("tcp", addr)
	fmt.Printf("SSH: listening\n")
	if err != nil {
		return err
	}
	return srv.Serve(ln)
